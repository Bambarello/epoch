...
txs_gc(Config) ->
    ...
    {ok, _}    = add_spend_tx(N1, 1000, 20000,  8,  5),  %% Short TTL - expires at 5 + 2 = 7
...
check_coinbase_validation(Config) ->
    ...
    aecore_suite_utils:connect(N1),
    {ok, TxH1, Ct1, Code} =
        create_contract_tx(N1, chain, <<"()">>,  200000,  1,  100),
    {ok, TxH2} =
        call_contract_tx(N1, Ct1, Code, <<"save_coinbase">>, <<"()">>, 200000,  2,  100),
    {ok, _} =
