     ...
     [{"No txs in mempool",
      ...
      {"Ensure ordering",
       fun() ->
                 ...
                 PK4 = new_pubkey(),
                 STxs =
                   [ a_signed_tx        (_Sender=PK1, me,_Nonce=1,_Fee=200000)
                   , a_signed_tx        (        PK1, me,       2,     300000)
                   , a_signed_tx        (        PK1, me,       3,     400000)
                   , a_signed_tx        (        PK2, me,       2,     600000)
                   , a_signed_tx        (        PK2, me,       1,     700000)
                   , signed_ct_create_tx(        PK4,           1,     200000,_GasPrice=1100000000)
                   , signed_ct_call_tx  (        PK4,           2,     500000,          9000000000)
                   , signed_ct_call_tx  (        PK4,           3,     800000,          1)
                   ],
      ...
      {"Ensure candidate ordering",
       fun() ->
               ...
               STx6 = signed_ct_call_tx(PK, _Nonce6=3,_Fee6=250000,_GasPrice6=9000000000),
      ...
      {"Ensure block gas limit",
       fun() ->
               ...
               STx2 = signed_ct_create_tx(PK2,    1, 200000,  1000),
               STx3 = signed_ct_call_tx(  PK3,    1, 200000,  1000),
      ...
      {"Test rejection of transactions",
       fun() ->
               ...
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 10, 200000, 1))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 20, 200000, 1))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 11, 200000, 2))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 21, 200000, 2))),
               meck:expect(aec_governance, minimum_gas_price, 0, 2),
               ?assertEqual({error, too_low_gas_price}, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 12, 400000, 0))),
               ?assertEqual({error, too_low_gas_price}, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 22, 400000, 0))),
               ?assertEqual({error, too_low_gas_price}, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 13, 400000, 1))),
               ?assertEqual({error, too_low_gas_price}, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 23, 400000, 1))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 14, 400000, 2))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 24, 400000, 2))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_create_tx(PubKey1, 15, 600000, 3))),
               ?assertEqual(ok, aec_tx_pool:push(signed_ct_call_tx  (PubKey1, 25, 600000, 3))),
